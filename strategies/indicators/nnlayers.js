...
var Indicator = function(settings) {
	...
	this.scale = 1;
	this.prediction = 0;
	this.file_name = settings.file_name;
	...
	for(let i = 0; i < settings.numOfLayers; i++){
		if(settings.activation == 'none'){
			layers.push({type:'fc', num_neurons: settings.neurons})
		} else {
			layers.push({type:'fc', num_neurons: settings.neurons, activation: settings.activation});
		}		
	}
	...
	let x = fs.existsSync(this.file_name);
	if(x == true && settings.shouldLoad == 'true') {
		let y = fs.readFileSync(this.file_name, 'utf8');
		let json = JSON.parse(y);
		this.nn.fromJSON(json);
	}
	else {
...
Indicator.prototype.update = function(candle) {
  ...
  if (1 === this.scale && 1 < candle.high && 0 === this.priceBuffer.length) this.setNormalizeFactor(candle);
...
Indicator.prototype.saveTrainer = function() {
	let json = this.nn.toJSON();
	let str = JSON.stringify(json);
	let x = fs.existsSync(this.file_name);
	fs.writeFileSync(this.file_name, str);
}
